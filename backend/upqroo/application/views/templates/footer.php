<footer class="flex justify-center">
    <div class="column">
        <div class="footer-main justify-center">
            <div class="container">
                <div class="row-responsive">
                    <div class="left wow fadeInLeft">
                        <div class="column">
                            <div class="white-space-32"></div>
                            <h3 class="color-white">UNIVERSIDAD</h3>
                            <div class="quick-links">
                                <div class="column">
                                    <div class="white-space-8"></div>
                                    <div class="quick-link">
                                        <a class="footer-link" href="<?php echo base_url();?>nosotros">NOSOTROS</a>
                                    </div>
                                    <div class="quick-link">
                                        <a class="footer-link" href="<?php echo base_url();?>directorio">DIRECTORIO</a>
                                    </div>
                                    <div class="quick-link">
                                        <a class="footer-link" href="<?php echo base_url();?>admision">ADMISIÓN</a>
                                    </div>
                                </div>
                            </div>
                            <div class="white-space-32"></div>
                        </div>
                    </div>
                    <div class="middle wow fadeInUp">
                        <div class="column justify-center">
                            <div class="white-space-32"></div>
                            <div class="responsive-img logo">
                                <img src="<?php echo base_url();?>assets/img/upqroo-newlogo.png" alt="Logo UPQROO" title="Logo UPQROO">
                            </div>
                            <div class="white-space-32"></div>
                        </div>

                    </div>
                    <div class="right justify-end">
                        <div class="column align-end">

                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="footer-bottom justify-center wow fadeInUp" data-wow-delay="0.3s">
            <div class="container">
                <div class="column">
                    <div class="white-space-8"></div>
                    <div class="row">
                        <div class="social justify-center">
                            <a rel="follow" class="element text-center btn" href="https://www.facebook.com/upqroo"><i class="social-icon-dark fab fa-facebook-f"></i></a>
                            <a rel="follow" class="element text-center btn" href="https://twitter.com/upqroo"><i class="social-icon-dark fab fa-twitter"></i></a>
                            <a rel="follow" class="element text-center btn" href="https://www.youtube.com/channel/UCy6VTWYIB9tay8P28cPe6Eg"><i class="social-icon-dark fab fa-youtube"></i></a>
                            <a rel="follow" class="element text-center btn" href="mailto:info@upqroo.edu.mx"><i class="social-icon-dark fa fa-envelope"></i></a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="address justify-center">
                            <p class="text-center textTiny">Av. Arco Bicentenario, Mza. 11, Lote 1119-33, Sm. 255. Cancún, Quintana Roo, México. C.P. 77500 <br> Tel. y Fax (998) 283 1859</p>
                        </div>
                    </div>
                    <div class="white-space-32"></div>
                </div>
            </div>
        </div>
</footer>

    <script type="text/javascript" src="<?php echo base_url();?>assets/js/main.js" async></script>
    <script type="text/javascript" src="<?php echo base_url();?>assets/js/wow.min.js" async></script>
    <script type="text/javascript">
        new WOW().init();
    </script>
</body>
</html>
